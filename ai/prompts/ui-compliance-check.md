...
8. **Findings** — each with severity, the related `UIAC-nn` criterion ID from
   feature spec §11b, description, and recommended correction. If a finding is
   not related to a UI acceptance criterion, state `N/A — no related UIAC ID`.
...
- findings related to feature-spec UI acceptance criteria must cite the stable
   `UIAC-nn` ID, not only the section number
