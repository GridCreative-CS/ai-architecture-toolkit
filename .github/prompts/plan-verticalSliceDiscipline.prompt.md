...
1. Re-run the delivery planner with updated prompt against the Atlas architecture → verify it does NOT produce a separate frontend slice
2. Generate a feature spec for one slice → verify "Human Workflow Surfaces" is populated
3. Run updated architecture compliance against the original (flawed) delivery plan → verify it flags frontend-as-a-slice as a verticality violation
4. Apply updated Definition of Ready to original Slice 4 (Email Outreach) → verify it fails because it lacks the human approval workflow surface
