...
17. All implementation code follows `ai/guides/code-quality-standard.md`: read nearby code and tests before writing, follow existing project patterns over model defaults, no new libraries or speculative abstractions without justification, no silent contract changes (public API, database/schema, events, UI), no TODOs/placeholders/fake implementations. Stable feature-spec criterion IDs flow through the OVERVIEW Requirement Coverage Map and Part Quality Report §3b; applicable mutation checks are recorded. Every Part ends with a Part Quality Report (`ai/templates/code-quality-checklist-template.md`) and passes the Part code review (engineering workflow Step 6a, `ai/prompts/code-quality-reviewer.md`) with all twelve checks, including the D1–D9 dimension and requirement coverage audits, before the next Part starts.
