...
[![Version](https://img.shields.io/badge/version-4.3.0-blue?style=flat-square)](VERSION.md)
...
| `design-system-completeness-gate` | Gate a design system on renderability and computed contrast before it becomes authoritative |
...
- `ui-foundation-workflow.md` — Greenfield UI — create and gate the design system before delivery planning
- `ui-retrofit-workflow.md` — Retrofit UI — inventory, derive and gate the design system, migrate existing slices
...
- `design-system.md` — Design system (authoritative for UI decisions once gated)
- `design-system-gate.md` — Design system completeness gate report (UI foundation Step 1b / UI retrofit Step 2b)
...
A .NET [Model Context Protocol](https://modelcontextprotocol.io/) server that exposes the toolkit to LLM-powered environments: toolkit content as resources (`toolkit://…`, including the execution skills at `toolkit://skills/…`), project artifacts as resources (`project://…`, including decomposition output at `project://ai-parts/{sliceId}` with per-Part status and Step 6a review verdicts), and tools such as `get_workflow_context`, `get_slice_context`, and `check_verticality`. See `mcp-server/README.md`. Using it is optional — the toolkit works with plain file access.
...
Both tracks produce `architecture/design-system.md` and then gate it (`ai/prompts/design-system-completeness-gate.md` → `architecture/design-system-gate.md`); it becomes authoritative for UI decisions once that gate records `APPROVED` or `APPROVED WITH NOTES`. Projects without UI can skip both tracks entirely — all UI-related steps are conditional on the project type.
